...
cuisines.map(cuisine=>{
  ...
  btn.style.border = "1px solid #7d777e"
  btn.style.marginLeft ="15px"
  btn.style.background="#F8D3FF"
  btn.classList.add("btn", "btn-item","btn-container",cuisine)
  btnContainer.appendChild(btn)
})

//! butonDOM ELEMENT SEÇİMİ
...
const showMenu=(menuCategory)=>{

    if(menuCategory=="All"){
      console.log(menu)
      menuSection.innerHTML=" "
      menu.map((item=>{
        menuSection.innerHTML +=  `<div class="menu-items col-lg-6 col-sm-12">
        <img
          src=${item.img}
          alt=${item.title}
          class="photo"
  ...
  //! Category all ise hepsini değilse filter methodu kullanılarak ilgili kategorinin yemeklerinin gösterilmesi
  else {
    let filterMenu = menu.filter((items) => items.category === menuCategory)
...
//! sayfa ilk yüklendiğinde tüm menülerin menülerin çıkması
window.addEventListener("DOMContentLoaded", () => showMenu("All"))

//! butona tıklandığında ilişkili menünün gösterimi
